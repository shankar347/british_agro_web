...
const API_ENDPOINT = `${API_BASE_URL}/platform-unified-log`;
const API_FETCH_ENDPOINT = `${API_BASE_URL}/platform-unified-log/uni`;
...
const createEmptySubstage = (name, includeMoisture = true) => ({
    name: name,
    startDate: '',
    startTime: '',
    endDate: '',
...
const createEmptyBatch = (batchNumber, materials = [], batchId = null, plainBunkerMixed = false) => {
    const batch = {
        id: batchId || Date.now(),
    ...
    materials.forEach(material => {
        ...
        if (apiStrawType !== undefined) {
            batch.materials.push({
                ...
                plainBunkerSubstage: createEmptySubstage(PLAIN_BUNKER_STAGE, true)
            });
        }
    });
...
const hasSubstageData = (substage) => {
    return substage.startDate || substage.startTime || substage.endDate || substage.endTime || substage.moisture || substage.remarks;
};
...
const hasCompleteMaterialData = (material, includePlainBunker = false) => {
    ...
    const hasMainData = material.substages.some(substage =>
        substage.startDate || substage.startTime || substage.endDate || substage.endTime || substage.moisture || substage.remarks
    );
    ...
    if (includePlainBunker && material.plainBunkerSubstage) {
        ...
        const hasPbData = pb.startDate || pb.startTime || pb.endDate || pb.endTime || pb.moisture || pb.remarks;
...
const convertToApiPayload = (batch, material, isCompleted) => {
    ...
    const longHeap = material.substages[0] || createEmptySubstage("Long Heap", true);
    const longHeapRest = material.substages[1] || createEmptySubstage("Long Heap Rest", true);
    const dryMixing = material.substages[2] || createEmptySubstage("Dry Mixing", true);
    const dryMixingRest = material.substages[3] || createEmptySubstage("Dry Mixing Rest", true);
    const plainBunker = material.plainBunkerSubstage || createEmptySubstage("Plain Bunker", true);
    ...
    const long_heap_start = (longHeap.startDate && longHeap.startTime)
        ? combineDateTime(longHeap.startDate, longHeap.startTime)
        : null;
    const long_heap_end = (longHeap.endDate && longHeap.endTime)
        ? combineDateTime(longHeap.endDate, longHeap.endTime)
        : null;

    const long_heap_rest_start = (longHeapRest.startDate && longHeapRest.startTime)
        ? combineDateTime(longHeapRest.startDate, longHeapRest.startTime)
        : null;
    const long_heap_rest_end = (longHeapRest.endDate && longHeapRest.endTime)
        ? combineDateTime(longHeapRest.endDate, longHeapRest.endTime)
        : null;

    const dry_mixing_start = (dryMixing.startDate && dryMixing.startTime)
        ? combineDateTime(dryMixing.startDate, dryMixing.startTime)
        : null;
    const dry_mixing_end = (dryMixing.endDate && dryMixing.endTime)
        ? combineDateTime(dryMixing.endDate, dryMixing.endTime)
        : null;

    const dry_mixing_rest_start = (dryMixingRest.startDate && dryMixingRest.startTime)
        ? combineDateTime(dryMixingRest.startDate, dryMixingRest.startTime)
        : null;
    const dry_mixing_rest_end = (dryMixingRest.endDate && dryMixingRest.endTime)
        ? combineDateTime(dryMixingRest.endDate, dryMixingRest.endTime)
        : null;
    ...
    const allStarts = [
        long_heap_start,
        long_heap_rest_start,
        dry_mixing_start,
        dry_mixing_rest_start,
        plain_bunker_start
    ...
    const payload = {
        ...
        plain_bunker: {
            ...
            moisture: plainBunker.moisture ? parseInt(plainBunker.moisture) : null
        },
        long_heap: {
            start_time: formatDateTime(long_heap_start),
            end_time: formatDateTime(long_heap_end),
            moisture: longHeap.moisture ? parseInt(longHeap.moisture) : null
        },
        long_heap_rest: {
            start_time: formatDateTime(long_heap_rest_start),
            end_time: formatDateTime(long_heap_rest_end),
            moisture: longHeapRest.moisture ? parseInt(longHeapRest.moisture) : null
        },
        dry_mixing: {
            start_time: formatDateTime(dry_mixing_start),
            end_time: formatDateTime(dry_mixing_end),
            moisture: dryMixing.moisture ? parseInt(dryMixing.moisture) : null
        },
        dry_mixing_rest: {
            start_time: formatDateTime(dry_mixing_rest_start),
            end_time: formatDateTime(dry_mixing_rest_end),
            moisture: dryMixingRest.moisture ? parseInt(dryMixingRest.moisture) : null
        }
    };
...
const convertApiResponseToUI = (apiData, batch) => {
    ...
    batch.materials.forEach(material => {
        ...
        const substageMapping = [
            { name: "Long Heap", data: materialData.long_heap },
            { name: "Long Heap Rest", data: materialData.long_heap_rest },
            { name: "Dry Mixing", data: materialData.dry_mixing },
            { name: "Dry Mixing Rest", data: materialData.dry_mixing_rest }
        ];
        ...
        if (materialData.plain_bunker) {
            ...
            if (pbData.moisture !== undefined && pbData.moisture !== null) {
                material.plainBunkerSubstage.moisture = pbData.moisture.toString();
            }
...
const MaterialTable = React.memo(({ material, onUpdate, showPlainBunker = false, readOnly = false }) => {
    ...
    const getTableTitle = () => {
        return material.plainBunkerMixed === true
            ? `${material.displayName} Platform Process (Mixed)`
            : `${material.displayName} Platform Process`;
    };
                            ...
                            <tr key={index}>
                                ...
                                <td>
                                    <input
                                        ...
                                        onChange={(e) => handleInputChange(index, 'moisture', e.target.value, false)}
                        ...
                        {showPlainBunker && (
                            <tr className="plain-bunker-row">
                                ...
                                <td>
                                    <input
                                        type="number"
                                        value={material.plainBunkerSubstage?.moisture || ''}
                                        onChange={(e) => handleInputChange(null, 'moisture', e.target.value, true)}
                                        className="table-input moisture-input"
                                        placeholder="%"
                                        min="0"
                                        max="100"
...
function BatchSelection({ onSelectBatch, loading, error, initialBatchNumber, initialBatchId }) {
    ...
    const handleLoadBatch = async (batchNum, batchId) => {
        ...
        try {
            ...
            const plainBunkerMixed = selectedBatch.plain_bunker_mixed === true;

            const newBatch = createEmptyBatch(batchNum, materials, selectedBatch.id, plainBunkerMixed);
    ...
    return (
        <div className="batch-selection">
            <h2>Unified Platform Process Entry</h2>
...
export default function UnifiedPlatformContent() {
    const router = useRouter();
    const toast = useToast();
    const searchParams = useSearchParams();
    ...
    const validateBatchData = (batch, includePlainBunker = false) => {
        for (const material of batch.materials) {
            ...
            if (includePlainBunker && material.plainBunkerSubstage) {
                const pb = material.plainBunkerSubstage;
                const hasPbData = pb.startDate || pb.startTime || pb.endDate || pb.endTime || pb.moisture;
                ...
                <div className="entry-header">
                    <div className="header-left">
                        ...
                        <span className="batch-status">
                            {currentBatch.plainBunkerMixed ? '(Mixed)' : '(Not Mixed)'}
                        </span>
                    </div>
